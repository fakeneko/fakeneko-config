...
一个轻量、类似 MaLiLib 的 Minecraft 配置库，支持 Fabric 与 NeoForge 双平台。它提供纯 Java API、Gson JSON 序列化、自动配置 GUI 以及组合键热键支持。
...
从开发者友好度看，`fakeneko_config` 比传统的 Cloth Config / YACL 方案更简洁。

### 对比

| 方面 | 旧的 ScreenBuilder / ScreenBuilderYacl | fakeneko_config |
| ------ | ---------------------------------------- | ----------------- |
| 代码量 | 需要写两套 GUI 代码（Cloth + YACL），并判断加载哪个库 | 注册配置项即可，自动渲染 |
| 依赖管理 | 依赖 Cloth Config 和 YACL，需要处理版本兼容和 optional/suggests | 统一使用 `fakeneko_config` 的 GUI，外部依赖少 |
| 配置类型扩展 | 每新增一种类型要自己写 GUI 绑定 | 库已经支持 Boolean、Hotkey 等类型，直接 new |
| 序列化 | 自己写 Gson 保存/加载 | 库自动保存/加载 |
| 热键 | 自己用 `KeyMapping` 注册和监听 | 库自动监听键盘/鼠标事件，`BooleanConfig.withHotkey` 一键绑定 |
| 配置界面入口 | 自己写 Fabric ModMenu / NeoForge ModList 适配 | 统一 `ConfigScreen` 即可 |
...
然后要在两个 ScreenBuilder 里分别写绑定，还要自己管理 JSON 保存。
...
- [构建与打包](#构建与打包)
- [项目结构](#项目结构)
- [支持的 Minecraft 版本](#支持的-minecraft-版本)
- [许可证](#许可证)

## 支持平台

支持 Fabric 与 NeoForge 双平台。本库只依赖 Minecraft 与 Gson，不依赖 Fabric API 或 NeoForge API 本身。
...
`ConfigScreen` 的 `Done` 按钮会自动调用 `MANAGER.save()`。
