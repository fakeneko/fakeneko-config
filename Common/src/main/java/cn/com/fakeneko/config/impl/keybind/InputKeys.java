...
public class InputKeys extends AbstractCollection<InputConstants.Key> {
	...
	public InputKeys(List<InputConstants.Key> keys) {
		this.keys = keys.stream().distinct().collect(Collectors.toUnmodifiableList());
	}

	public InputKeys(InputConstants.Key[] keys) {
		this.keys = Arrays.stream(keys).distinct().collect(Collectors.toUnmodifiableList());
	}

	public boolean isLastKey(InputConstants.Key key) {
		return !this.keys.isEmpty() && this.keys.get(this.keys.size() - 1).equals(key);
	}
	...
	public static InputKeys ofKeys(int ...keys) {
		return new InputKeys(IntStream.of(keys).mapToObj(InputConstants.Type.KEYSYM::getOrCreate).collect(Collectors.toUnmodifiableList()));
	}

	public static InputKeys ofMouse(int ...buttons) {
		return new InputKeys(IntStream.of(buttons).mapToObj(InputConstants.Type.MOUSE::getOrCreate).collect(Collectors.toUnmodifiableList()));
	}
	...
	public String toString() {
		...
		return String.join(SEPARATOR_STR, this.keys.stream().map(InputKeys::toShortName).collect(Collectors.toList()));
	}
